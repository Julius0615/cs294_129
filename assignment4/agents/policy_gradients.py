...
class PolicyGradient(object):
    ...
    def discount_rewards(self, r, do_reset=False):
        ...
        for i in xrange(r.shape[0] - 1, -1, -1):
            if do_reset and r[i] != 0:
                total = 0
            discounted_r[i] = r[i] + self.gamma * total
            total = r[i] + self.gamma * total
        ########################################################################
        #                           END OF YOUR CODE                           #
        ########################################################################
    ...
    def policy_backward(self, ep_x, ep_h, ep_dprobs, discounted_epr):
        ...
        T, D = ep_x.shape
        T, H = ep_h.shape
        d_sigmoid = ep_dprobs * discounted_epr # (T, 1)
        dW2 = np.dot(ep_h.T, d_sigmoid).flatten()
        dh = np.dot(d_sigmoid, self.model['W2'].reshape(1, -1)) # (T, H)
        dh[ep_h == 0] = 0
    ...
    def train(self, environment="CartPole-v0", max_episodes=100, print_every=1):
        ...
        while True:
            ...
            ####################################################################
            dprobs.append(
                1.0 / (y * aprob + (1 - y) * (1 - aprob)) * (2 * y - 1) * aprob * (1 - aprob)
            )
            ####################################################################
            #                        END OF YOUR CODE                          #
            ####################################################################
